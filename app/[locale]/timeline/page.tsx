...
export async function generateMetadata(
  ...
  return {
    ...
    description: isJa
      ? '1873年の創業から2002年のMade in USA終了まで。Big E・ケアラベル・セルビッジ・TALON・日付コードなど鑑定に直結するLevi\'sの変遷を年表で解説。'
...
const TIMELINE: Milestone[] = [
  ...
  {
    year: 1936,
    labelJa: '赤タブ導入・506XX（Type1ジャケット）誕生',
    labelEn: 'Red Tab Introduced — 506XX (Type 1 Jacket) Born',
    descJa: 'バックポケットに赤い布製タブ「Red Tab」が商標として導入される（両面大文字「LEVIS」＝Big E仕様の始まり）。同年、初のデニムジャケット506XX（Type1）も誕生。',
    descEn: 'The red fabric "Red Tab" trademark is introduced on back pockets — both sides uppercase "LEVIS" marks the beginning of the Big E specification. The first denim jacket, 506XX (Type 1), also debuts.',
    type: 'detail',
  ...
  {
    year: 1953,
    labelJa: '507XX（Type2ジャケット）誕生',
    labelEn: '507XX (Type 2 Jacket) Debuts',
    descJa: '506XXの後継モデルとして507XX（Type2）が登場。胸ポケットが2つになり、ウエスト部にダブルプリーツが追加。1962年まで製造された希少モデル。',
    descEn: '507XX (Type 2) debuts as the 506XX\'s successor. Two chest pockets and double waist pleats added. Produced until 1962 — a rare model.',
    type: 'product',
  ...
  {
    year: 1962,
    labelJa: '「Every Garment Guaranteed」廃止・557XX誕生',
    labelEn: '"Every Garment Guaranteed" Removed — 557XX Trucker Jacket Born',
    descJa: 'JacronパッチのEGGスローガンが廃止。同年、557XX（トラッカージャケット・Type3）が誕生し、現在まで継続生産されている。Big E期（〜1971年）の品番は「70557XX」。',
    descEn: '"Every Garment Guaranteed" slogan removed from Jacron patches. The 557XX Trucker Jacket (Type 3) debuts and continues to the present. Big E era (pre-1971) examples carry the "70557XX" number.',
    type: 'product',
    articleSlug: 'article-jacket-types',
    articleLabelJa: 'ジャケット完全ガイド',
    articleLabelEn: 'Jacket Complete Guide',
  ...
  },
  {
    ...
    descJa: '【最重要転換点】1971年7月：FTCのCare Labeling Ruleによりケアラベルが義務化。赤タブが両面大文字（Big E）から片面小文字（Small e）に変更。YKKジッパーへの移行が進む。この年が鑑定の最大の分岐点。',
    descEn: '[MOST CRITICAL TURNING POINT] July 1971: FTC Care Labeling Rule makes care labels mandatory. Red tab changes from both-sides uppercase Big E to one-side lowercase Small e. YKK zipper adoption accelerates. This year is the single biggest dating dividing line.',
    type: 'law',
  ...
  {
    year: 1981,
    labelJa: 'セルビッジデニム廃止（ロータリー織機へ移行）',
    labelEn: 'Selvedge Denim Discontinued — Rotary Loom Transition',
    descJa: 'シャトル織機からロータリー織機への完全移行により、耳付きセルビッジデニムが廃止。セルビッジあり＝1981年以前の確定指標。1981〜1984年はアウトシームがオレンジ色のオーバーロック。',
    descEn: 'Full transition from shuttle to rotary looms discontinues selvedge denim. Selvedge present = confirmed pre-1981. Outseam overlock is orange from 1981–1984.',
    type: 'detail',
  ...
  return (
    <>
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-6 py-16">
        ...
        <p className="text-sm text-[#b8cce4] font-light leading-loose mb-10">
          {t(
        ...
        {/* タイムライン本体 */}
        {DECADES.map(decade => {
          const items = TIMELINE.filter(m => m.year >= decade.start && m.year <= decade.end);
