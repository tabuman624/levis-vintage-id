...
export default async function ArticlesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const cats = [...new Set(ARTICLES.map(a => locale === 'ja' ? a.catJa : a.catEn))];
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-16">
        ...
        {cats.map(cat => (
          <div key={cat} className="mb-12">
            <p className="font-mono text-[9px] tracking-[4px] text-stitch/80 uppercase mb-4 pb-3 border-b border-stitch/10">{cat}</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {ARTICLES.filter(a => (locale === 'ja' ? a.catJa : a.catEn) === cat).map(a => (
                <Link key={a.slug} href={p(`/articles/${a.slug}`)} className="card-denim p-5 flex flex-col gap-2 no-underline">
                  <h3 className="text-sm font-medium text-[#f0ebe0] leading-snug">{locale === 'ja' ? a.titleJa : a.titleEn}</h3>
                  <p className="text-xs text-fade font-light leading-relaxed">{locale === 'ja' ? a.descJa : a.descEn}</p>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {a.tags.map(tag => (
                      <span key={tag} className="font-mono text-[8px] text-[#1a2a3a] bg-stitch/60 px-2 py-0.5 rounded">{tag}</span>
                    ))}
                  </div>
                </Link>
              ))}
